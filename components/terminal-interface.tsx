...
export function TerminalInterface({
  ...
  const isContactlessBelowPinThreshold = () => {
    ...
    console.log(
      `Amount in UYU: ${amountInUYU}, Threshold: ${CONTACTLESS_NO_PIN_THRESHOLD_UYU}`
    );
    ...
    <div className="flex flex-col h-full">
      ...
      {selectedCard ? (
        ...
        <div className="bg-yellow-100 p-3 rounded-lg border border-yellow-300 mb-4 text-center">
          ...
          <p className="text-sm text-yellow-700">
            Please select a card from the wallet
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-4 mt-4">
        ...
        >
          ...
          {isContactlessBelowPinThreshold() && (
            <span className="text-xs text-green-600 mt-1">No PIN required</span>
          )}
        </div>
    ...
    <>
      <Dialog open={isTerminalOpen} onOpenChange={setIsTerminalOpen}>
        ...
        <DialogContent
          ...
          style={{ zIndex: 50000 }}
        >
          <DialogTitle className="sr-only">POS Terminal</DialogTitle>
          <div className="flex flex-col h-full">
            ...
            <div className="bg-gray-200 flex-1 p-4 font-mono text-black relative overflow-hidden border-8 border-gray-800">
                  ...
                  {networkConfig?.enabled && (
                    <div className="mt-4 text-xs text-center bg-green-100 p-2 rounded-lg border border-green-300">
                      ...
                      <span className="text-green-700">
                        Transactions will be sent to {networkConfig.host}:
                        {networkConfig.port}
                      </span>
                    </div>
                  )}
                ...
                <div className="flex flex-col h-full">
                  <div className="text-center font-bold border-b border-gray-400 pb-2 mb-4">
                    ...
                    <div className="text-xs font-normal">
                      {selectedTransactionType.charAt(0).toUpperCase() +
                        selectedTransactionType.slice(1)}
                    </div>
                  </div>
              ...
              {step === "currency" && (
                <div className="flex flex-col h-full">
                  ...
                  <div className="flex flex-col gap-3">
                    {currencies
                      .slice(menuIndex * 4, menuIndex * 4 + 4)
                      .map((currency, idx) => (
                        <div
                          key={currency.code}
              ...
              {step === "payment-action" && (
                <div className="h-full">
                  {paymentMethod === "chip" && (
                    <ChipReader onInsert={() => handlePaymentStart("chip")} />
                  )}

                  {paymentMethod === "contactless" && (
                    <ContactlessReader
                      onTap={() => handlePaymentStart("contactless")}
                    />
                  )}

                  {paymentMethod === "swipe" && (
                    <CardSwipe onSwipe={() => handlePaymentStart("swipe")} />
                  )}
                </div>
              )}
              ...
              {step === "pin" && (
                <div className="flex flex-col h-full">
                  ...
                  {pinError && (
                    <div className="text-center text-red-600 font-bold animate-pulse my-2">
                      INCORRECT PIN
                    </div>
                  )}
                  <div className="mt-auto text-xs text-center">
                ...
                <div className="flex flex-col items-center justify-center h-full">
                  {processingState === "approved" ? (
                    <div className="text-center">
                      ...
                      <div className="animate-pulse text-xs mt-4">
                        Printing receipt...
                      </div>
                    </div>
                  ) : (
            ...
            <div className="bg-gray-800 p-4">
              ...
              <div
                className={cn(
                  ...
                  showKeypad ? "opacity-100" : "opacity-50 pointer-events-none"
                )}
              >
                {[1, 2, 3, 4, 5, 6, 7, 8, 9, 0].map((key) => (
                  <Button
                    ...
                    onClick={() =>
                      step === "pin"
                        ? handlePinKeyPress(key.toString())
                        : handleAmountKeyPress(key.toString())
                    }
                  >
                    {key}
                  </Button>
                ...
                <Button
                  ...
                  onClick={() =>
                    step === "pin"
                      ? handlePinKeyPress("00")
                      : handleAmountKeyPress("00")
                  }
                >
                  00
                </Button>
                ...
                <Button
                  ...
                  onClick={() =>
                    step === "pin"
                      ...
                      : handleAmountKeyPress("backspace")
                  }
                >
                  ←
                </Button>
                <Button
                  ...
                  onClick={() =>
                    step === "pin"
                      ? handlePinKeyPress("enter")
                      : handleAmountKeyPress("enter")
                  }
                >
                  ENTER
                </Button>
